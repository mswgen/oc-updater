        ...
        return new Promise(resolve => {
            ...
            */
            ...
            ipc.on('alert-closed', (_event: any, pid: number, id: string) => {
                if (id == '65' && pid == PID) {
                    if (fs.existsSync(`${ocDir}/Bootstrap`)) fs.rmSync(`${ocDir}/Bootstrap`, { recursive: true });
            ...
            if (plistParsed.Misc.Security.BootProtect == 'Bootstrap' || plistParsed.Misc.Security.BootProtect == 'BootstrapShort') hasBootstrap = true;
            if (!hasBootstrap) {
                fs.writeFileSync(file, plist.build(plistParsed));
                resolve(null);
            } else webContents.send('alert', PID, '65', `경고: Bootstrap.efi 감지됨
