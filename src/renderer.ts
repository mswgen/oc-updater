...
Do you want to contnue?`)) ipc.send('quit');
    ...
    await sleep(100);
    ipc.sendSync('toggle-beta', isBeta);
    await sleep(1000);
...
function afterOcverSelection(ocver: string, efidir: string) {
    ...
    if (ocverNum < 81) {
        ...
        document.querySelector('#update')?.addEventListener('click', () => updateOC(ocverNum, kexts, false));
    }
    document.querySelector('#updatebeta')?.addEventListener('click', () => updateOC(ocverNum, kexts, true));
}
